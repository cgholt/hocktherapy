...
export async function POST(req: NextRequest) {
  ...
  try {
    // Plain-text only, and no external reply-to: ProtonMail's outbound
    // SpamAssassin blocks HTML / reply-to-heavy messages (550 5.7.0). A clean
    // text message matches what delivers reliably. The visitor's email is in the
    // body so it can still be replied to manually.
